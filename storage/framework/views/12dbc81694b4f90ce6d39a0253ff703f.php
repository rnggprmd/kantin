<?php $__env->startSection('title', 'Nota ' . $transaksi->kode_transaksi); ?>
...
<div class="space-y-8">
    ...
    <div class="bg-white rounded-[2rem] border border-gray-200 shadow-sm p-4 lg:p-5">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="flex items-center gap-5">
                ...
                <div class="space-y-1">
                    <div class="flex items-center gap-3">
                        <h2 class="text-xl font-black text-gray-900 tracking-tighter"><?php echo e($transaksi->kode_transaksi); ?></h2>
                    ...
                    <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                        ...
                        <?php echo e($transaksi->created_at->translatedFormat('d F Y, H:i')); ?>
            ...
            <div class="flex items-center gap-3">
                <a href="<?php echo e(route('transaksi.pdf', $transaksi->id)); ?>" class="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-3 bg-white border border-gray-200 text-gray-700 text-[10px] font-black rounded-xl hover:bg-gray-50 transition-all uppercase tracking-widest shadow-sm">
                    <span class="material-symbols-outlined !text-[18px]">picture_as_pdf</span>
                    Unduh PDF
                </a>

                <?php if($transaksi->status === 'pending' && $transaksi->metode_bayar === 'non_tunai' && $transaksi->snap_token): ?>
                ...
                <a href="<?php echo e(route('transaksi.create')); ?>" class="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-3 bg-primary text-white text-[10px] font-black rounded-xl hover:opacity-90 transition-all uppercase tracking-widest shadow-lg shadow-primary/10">
    ...
    <div class="bg-white rounded-[2.5rem] border border-gray-200 shadow-sm overflow-hidden mb-12">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 border-b border-gray-100 bg-white">
            <div class="p-6 border-r border-gray-100">
                ...
                <p class="text-sm font-black text-secondary uppercase tracking-tighter"><?php echo e(str_replace('_', ' ', $transaksi->metode_bayar)); ?></p>
            </div>
            <div class="p-6 border-r border-gray-100">
                <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Operator</p>
                <p class="text-sm font-black text-gray-900 truncate"><?php echo e($transaksi->user?->name ?? 'Sistem'); ?></p>
            </div>
            <div class="p-6">
                <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1.5">ID Referensi</p>
                <p class="text-sm font-black text-gray-900 font-mono tracking-tighter uppercase"><?php echo e(substr($transaksi->kode_transaksi, -8)); ?></p>
            </div>
        </div>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="group hover:bg-gray-50/50 transition-colors">
                        <td class="px-10 py-6">
                            <p class="text-sm font-black text-gray-800 tracking-tight"><?php echo e($detail->nama_menu); ?></p>
                        </td>
                        <td class="px-4 py-6 text-center">
                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-gray-100 text-xs font-black text-gray-600"><?php echo e($detail->qty); ?></span>
                        </td>
        ...
        <div class="p-10 bg-gray-900 flex flex-col md:flex-row justify-between items-start gap-12 relative overflow-hidden">
            ...
            <div class="flex-1 space-y-8 relative z-10">
                <div class="grid grid-cols-2 gap-8">
                    <div>
                        <h4 class="text-[9px] font-black text-white/30 uppercase tracking-[0.2em] mb-3">Informasi Transaksi</h4>
                        <div class="space-y-2">
                            <p class="text-xs text-white/70 font-black tracking-tight"><?php echo e($transaksi->kode_transaksi); ?></p>
                            <p class="text-[10px] text-white/40 font-bold uppercase tracking-widest"><?php echo e($transaksi->created_at->format('d/m/Y H:i')); ?></p>
                        </div>
                    </div>
                    <div>
                        <h4 class="text-[9px] font-black text-white/30 uppercase tracking-[0.2em] mb-3">Otorisasi Kantin</h4>
                        <div class="space-y-2">
                            <p class="text-xs text-white/70 font-black tracking-tight"><?php echo e($transaksi->user?->name ?? 'Sistem'); ?></p>
                            <p class="text-[10px] text-white/40 font-bold uppercase tracking-widest"><?php echo e($transaksi->created_at->format('H:i:s')); ?> WIB</p>
                        </div>
                    </div>
                </div>

                <?php if($transaksi->catatan && $transaksi->catatan !== '-'): ?>
                <div class="p-4 rounded-2xl bg-white/5 border border-white/10">
                    <p class="text-[9px] font-black text-white/30 uppercase tracking-widest mb-1.5">Catatan Pesanan:</p>
                    <p class="text-xs text-white/60 font-medium italic">"<?php echo e($transaksi->catatan); ?>"</p>
                </div>
                <?php endif; ?>
                ...
                <div class="flex items-center gap-6 pt-4">
                    ...
                    <div class="flex items-center gap-2">
                        <div class="w-2 h-2 rounded-full <?php echo e($transaksi->status === 'selesai' ? 'bg-secondary' : 'bg-white/10'); ?>"></div>
                        <span class="text-[9px] font-black text-white/40 uppercase tracking-widest">Paid</span>
                    </div>
                    <div class="w-8 h-[1px] bg-white/10"></div>
                    <div class="flex items-center gap-2">
                        <div class="w-2 h-2 rounded-full <?php echo e($transaksi->status === 'selesai' ? 'bg-green-500' : 'bg-white/10'); ?>"></div>
            ...
            <div class="w-full md:w-auto text-right relative z-10">
                <div class="mb-8">
                    ...
                    <p class="text-6xl font-black text-white tracking-tighter tabular-nums leading-none">Rp <?php echo e(number_format($transaksi->total_harga, 0, ',', '.')); ?></p>
                </div>
                ...
                <div class="space-y-2 pt-6 border-t border-white/5">
                    <div class="flex justify-between items-center gap-8 text-[11px] font-bold text-white/40 uppercase">
                        ...
                        <span class="text-white">Rp <?php echo e(number_format($transaksi->uang_bayar, 0, ',', '.')); ?></span>
                    </div>
                    <div class="flex justify-between items-center gap-8 text-[11px] font-bold text-white/40 uppercase">
                        <span>Sisa Kembali</span>
                        <span class="text-secondary">Rp <?php echo e(number_format($transaksi->kembalian, 0, ',', '.')); ?></span>
                    </div>
                </div>
                <?php endif; ?>
            </div>
...
<script src="https://app.<?php echo e(config('services.midtrans.is_production') ? '' : 'sandbox.'); ?>midtrans.com/snap/snap.js" data-client-key="<?php echo e(config('services.midtrans.client_key')); ?>"></script>
<script type="text/javascript">
    document.getElementById('pay-button').onclick = function(){
        snap.pay('<?php echo e($transaksi->snap_token); ?>', {
            onSuccess: function(result){
                fetch('<?php echo e(route('transaksi.mark-as-success', $transaksi->id)); ?>', {
                    method: 'POST',
                    headers: {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\kantin\resources\views/transaksi/show.blade.php ENDPATH**/ ?>
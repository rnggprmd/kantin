<?php $__env->startSection('title', 'Katalog Menu'); ?>
<?php $__env->startSection('page-title', 'Katalog Menu Kantin'); ?>

<?php $__env->startSection('content'); ?>
<div x-data="{ 
    showAdd: <?php if($errors->hasAny(['nama', 'kategori_id', 'harga', 'deskripsi', 'gambar']) && !old('_method')): ?> true <?php else: ?> false <?php endif; ?>, 
    showEdit: <?php if($errors->hasAny(['nama', 'kategori_id', 'harga', 'deskripsi', 'gambar']) && old('_method') === 'PUT'): ?> true <?php else: ?> false <?php endif; ?>,
    editData: {
        id: '<?php echo e(old('id')); ?>',
        nama: '<?php echo e(old('nama')); ?>',
        kategori_id: '<?php echo e(old('kategori_id')); ?>',
        harga: '<?php echo e(old('harga')); ?>',
        deskripsi: '<?php echo e(old('deskripsi')); ?>',
        is_tersedia: <?php echo e(old('is_tersedia') ? 'true' : 'true'); ?>
        ...
        <div class="px-5 py-3.5 bg-gray-50/50">
            <form method="GET" class="flex flex-col sm:flex-row gap-3 items-center">
                <div class="flex-1 relative w-full">
                    ...
                    <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari nama menu..."
                ...
                <div class="w-full sm:w-48 shrink-0">
                    <select name="kategori_id" class="w-full bg-white border border-gray-200 text-gray-700 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-secondary focus:ring-2 focus:ring-secondary/10 transition-all" onchange="this.form.submit()">
                        ...
                        <option value="<?php echo e($k->id); ?>" <?php echo e(request('kategori_id') == $k->id ? 'selected' : ''); ?>><?php echo e($k->nama); ?></option>
                ...
                <div class="w-full sm:w-48 shrink-0">
                    <select name="status" class="w-full bg-white border border-gray-200 text-gray-700 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-secondary transition-all" onchange="this.form.submit()">
                        ...
                        <option value="tersedia" <?php echo e(request('status') === 'tersedia' ? 'selected' : ''); ?>>Menu Tersedia</option>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4 xl:gap-6">
        ...
        <div class="bg-white border border-gray-100 hover:border-secondary/30 rounded-2xl flex flex-col overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 group relative">
            ...
            <div class="aspect-square bg-gray-50 relative overflow-hidden shrink-0">
                <?php if($menu->gambar): ?>
                <img src="<?php echo e(asset('storage/' . $menu->gambar)); ?>" alt="<?php echo e($menu->nama); ?>" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                <?php else: ?>
                <img src="https://loremflickr.com/300/300/food?lock=<?php echo e($menu->id); ?>" alt="<?php echo e($menu->nama); ?>" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                <?php endif; ?>
            ...
            <div class="p-3.5 flex flex-col flex-1">
                <div>
                    <h3 class="text-sm font-black text-gray-900 leading-tight line-clamp-1"><?php echo e($menu->nama); ?></h3>
                    <p class="text-[10px] text-secondary font-bold uppercase tracking-wider mt-0.5"><?php echo e($menu->kategori?->nama); ?></p>
                </div>
                <div class="mt-auto pt-3">
                    <p class="text-sm text-gray-900 font-black tracking-tight">Rp <?php echo e(number_format($menu->harga, 0, ',', '.')); ?></p>
                        ...
                        <div class="flex items-center gap-1">
                            <button type="button"
                               @click="showEdit = true; editData = { 
                                   ...
                                   nama: '<?php echo e($menu->nama); ?>', 
                                   kategori_id: '<?php echo e($menu->kategori_id); ?>', 
                                   harga: '<?php echo e($menu->harga); ?>', 
                                   deskripsi: '<?php echo e($menu->deskripsi); ?>',
                                   is_tersedia: <?php echo e($menu->is_tersedia ? 'true' : 'false'); ?>
                            ...
                            <button type="button"
                                    @click="$dispatch('confirm', {
                                        title: 'Hapus Menu?',
                                        message: 'Apakah Anda yakin ingin menghapus menu <?php echo e($menu->nama); ?>?',
                                        action: '<?php echo e(route('menu.destroy', $menu->id)); ?>',
                                        method: 'DELETE'
                                    })"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="nama" value="<?php echo e(old('nama')); ?>" required placeholder="Contoh: Nasi Goreng Spesial"
                                           class="w-full bg-gray-50 border <?php $__errorArgs = ['nama'];
...
unset($__errorArgs, $__bag); ?> text-gray-900 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-secondary focus:ring-2 focus:ring-secondary/10 transition-all">
...
unset($__errorArgs, $__bag); ?>
                                ...
                                <div>
                                    ...
                                    <select name="kategori_id" required class="w-full bg-gray-50 border <?php $__errorArgs = ['kategori_id'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php else: ?> border-gray-200 <?php unset($message);
...
unset($__errorArgs, $__bag); ?> text-gray-900 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-secondary focus:ring-2 focus:ring-secondary/10 transition-all">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="harga" value="<?php echo e(old('harga')); ?>" required placeholder="0"
                                           class="w-full bg-gray-50 border <?php $__errorArgs = ['harga'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php else: ?> border-gray-200 <?php unset($message);
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <textarea name="deskripsi" rows="3" placeholder="Jelaskan detail menu..."
                                              class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-secondary transition-all"><?php echo e(old('deskripsi')); ?></textarea>
                                </div>
    ...
    <div x-show="showEdit" x-cloak class="fixed inset-0 z-[110] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showEdit" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-gray-100">
                <form :action="'<?php echo e(url('menu')); ?>/' + editData.id" method="POST" enctype="multipart/form-data">
                    <?php echo csrf_field(); ?>
...
unset($__errorArgs, $__bag); ?>
                                ...
                                <div>
                                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1.5">Kategori</label>
                                    <select name="kategori_id" x-model="editData.kategori_id" required class="w-full bg-gray-50 border <?php $__errorArgs = ['kategori_id'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php else: ?> border-gray-200 <?php unset($message);
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\kantin\resources\views/menu/index.blade.php ENDPATH**/ ?>
...
@section('page-title', 'Modifikasi Basis Produk')

@section('content')
<div class="max-w-5xl mx-auto" x-data="{ 
    preview: '{{ $menu->gambar ? asset('storage/' . $menu->gambar) : '' }}', 
    isTersedia: {{ old('is_tersedia', $menu->is_tersedia ? 'true' : 'false') }} 
}">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-400 font-medium mt-1">Ubah atribut spesifikasi: <span class="text-primary font-bold">{{ $menu->nama }}</span></p>
            </div>
        </div>
    ...
    <form method="POST" action="{{ route('menu.update', $menu->id) }}" enctype="multipart/form-data" class="space-y-8">
        @csrf @method('PUT')
                ...
                <div class="bg-white rounded-3xl p-2 shadow-sm border border-gray-100 overflow-hidden relative group">
                    <div class="aspect-square bg-gray-50 rounded-2xl flex flex-col items-center justify-center border-2 border-dashed border-gray-200 group-hover:border-secondary transition-all relative overflow-hidden">
                        ...
                        <div class="text-center p-6" x-show="!preview">
                            ...
                            <p class="text-xs font-black text-gray-400 uppercase tracking-widest">Ganti Foto</p>
                        </div>

                        <img :src="preview" x-show="preview" x-cloak class="absolute inset-0 w-full h-full object-cover border border-gray-100 rounded-2xl">
                        
                        <div x-show="preview" x-cloak class="absolute inset-0 bg-black/40 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl">
                            <span class="material-symbols-outlined text-white !text-[24px]">sync</span>
                            <span class="text-[10px] font-black text-white uppercase mt-1">Klik Ganti Foto</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 space-y-6">
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div class="relative group">
                            ...
                            <div class="relative">
                                <select name="kategori_id" required
                                    ...
                                    <option value="{{ $k->id }}" {{ old('kategori_id', $menu->kategori_id) == $k->id ? 'selected' : '' }}>{{ $k->nama }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="relative group">
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Deskripsi</label>
                            <textarea name="deskripsi" rows="3"
                                      class="w-full bg-gray-50 border border-transparent text-gray-900 rounded-2xl px-5 py-3.5 text-sm font-bold focus:outline-none focus:bg-white focus:border-secondary transition-all resize-none">{{ old('deskripsi', $menu->deskripsi) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-8">
                <div class="relative group">
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Harga Jual</label>
                    <div class="relative">
                ...
                <div class="sm:col-span-2 bg-gray-50 rounded-2xl px-6 py-4 flex items-center justify-between border border-gray-100">
                    <div>
                        ...
                        <p class="text-xs text-gray-600 font-bold mt-0.5">Visibilitas menu di layar kasir.</p>
                    </div>
